...
function toggleScroll() {
    // 🔥 ELIMINAMOS EL ESCUDO: Ya no hay "if (countInActivo) return;"
    ...
    if (isScrolling) {
        ...
        if (countInActivo) {
            ...
            reproductorVozGuia.pause(); // Calla la voz inmediatamente
        }
...
function agregarAudioInput() {
    ...
    div.innerHTML = `
        ...
        <input type="file" id="file-Audio-${num}" accept="audio/*" style="display: none;" onchange="cargarAudioLocal(${num}, this)">
...
function arrancarScrollNativo() {
    ...
    window.ultimaDinamicaAnunciada = null; // 🔥 NUEVO: Reinicia la memoria de dinámicas al dar Play
    ...
    const runScroll = (time) => {
        ...
        try {
            ...
            if (typeof autoplayActivo !== 'undefined' && autoplayActivo && isAuto) {
                ...
                // Recolectamos TODOS los bloques, incluso si no tienen compás definido
                ...
                for (let i = 0; i < todosLosBloques.length; i++) {
                    ...
                    // 🔥 FIX: Umbral estricto para no cambiar de bloque antes de tiempo
                ...
                if (bloqueActivo) {
                    ...
                    // 🔥 HERENCIA INTELIGENTE: Si está en 0, copiamos el del bloque anterior
                    if (compases === 0) {
                        ...
                        if (compases === 0) compases = 1; // Fallback extremo
                    }
                    ...
                    // 🧠 EXTRACCIÓN SEGURA DEL TEXTO Y ESTADO DE REPETICIÓN
                    ...
                    // A) Anuncio Inicial (El "Intro" en el Beat 1)
                    ...
                    // B) Anuncio Anticipado A PRUEBA DE BALAS (Exactamente 2 beats antes del siguiente)
					...
					// --- LA VOZ GUÍA PARA DINÁMICAS (Paréntesis y Asteriscos) ---
                    ...
                    dinamicas.forEach(dyn => {
                        ...
                        let distanciaAnticipacion = targetY + (pxPorBeat * 2); // Lo anuncia 2 beats antes para prevenir
                    ...
                    // C) Parada Final Automática
                    if (!siguienteBloque) {
                        let ultimaLinea = bloqueActivo.lastElementChild || bloqueActivo;
                        if (ultimaLinea.getBoundingClientRect().bottom <= targetY) {
                            if (typeof toggleScroll === 'function') toggleScroll(); 
            ...
            // Seguridad: Si todo falla, toma la velocidad manual
            if (step === 0 || !autoplayActivo || !isAuto) {
        ...
        // Acumulador anti-congelamiento para móviles
...
// La inteligencia visual ahora está integrada 100% dentro de arrancarScrollNativo()
